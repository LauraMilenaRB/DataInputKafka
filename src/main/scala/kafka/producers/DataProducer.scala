...
import kafka.Constants.{BUCKET_NAME_STAGING,BUCKET_NAME_RAW, IP, SERVER_NAME, API_URL}
...
object DataProducer extends App {
  ...
  API_URL.keys.map(k => producerMessage(k))

  producer.close()

  private def producerMessage(url: String) = {
    ...
    ListOfDates.size match {
      case 1 => (ServiceBucketAws.createBucket(BUCKET_NAME_STAGING),ServiceBucketAws.createBucket(BUCKET_NAME_RAW)) match {
          case (Right(x1),Right(x2)) => LOGGER.info(s"$x1 and $x2")
            producer.send(new ProducerRecord[String, String](topic, key, value_topic)).get()
